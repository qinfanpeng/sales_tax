...
describe SalesTaxCalculator do
  ...
  describe 'only one' do
    context 'non-imported' do
      it 'exempt item' do
        calculator.items = ['2 books at 12.49']
        expect(calculator.run).to eq [
                                  '2 books: 24.98',
                                  'Sales Taxes: 0.00',
                                  'Total: 24.98']
      end
      it "non-exempt item" do
        calculator.items = ['2 music CDs at 14.99']
        expect(calculator.run).to eq [
                                  '2 music CDs: 32.98',
                                  'Sales Taxes: 3.00',
                                  'Total: 32.98']
    ...
    context 'imported' do
      it 'exempt item' do
        calculator.items = ['2 imported boxes of chocolates at 10.00']
        expect(calculator.run).to eq [
      ...
      it 'non-exempt item' do
        calculator.items = ['2 imported bottles of perfume at 47.50']
          expect(calculator.run).to eq [
